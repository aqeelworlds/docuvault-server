...
export async function getCategories(req, res) {
    try {
        ...
        const categories = await dbAll(`SELECT dt.*, 
       ...
       ORDER BY dt.is_custom ASC, dt.name ASC`, [userId || '', userId || '']);
        res.json({ categories });
    }
    catch (error) {
...
export async function createCustomCategory(req, res) {
    try {
        ...
        await dbRun('INSERT INTO document_types (id, name, slug, icon, color, is_custom, user_id) VALUES (?, ?, ?, ?, ?, 1, ?)', [categoryId, name.trim(), slug, icon || 'Folder', color || '#6366f1', userId]);
        res.status(201).json({
