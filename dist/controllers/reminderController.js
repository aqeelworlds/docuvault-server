...
export async function getReminders(req, res) {
    ...
    `;
        const reminders = await dbAll(sql, [userId]);
...
export async function toggleReminder(req, res) {
    try {
        ...
        const userId = req.user.id;
        const reminder = await dbGet('SELECT id, is_active FROM reminders WHERE id = ? AND user_id = ?', [reminderId, userId]);
        if (!reminder) {
...
export async function createCustomReminder(req, res) {
    try {
        const userId = req.user.id;
        ...
        const doc = await dbGet('SELECT id, expiry_date, user_id FROM documents WHERE id = ?', [documentId]);
        ...
        await dbRun('INSERT INTO reminders (id, document_id, user_id, lead_days, reminder_date, is_active) VALUES (?, ?, ?, ?, ?, 1)', [reminderId, documentId, userId, leadDays, reminderDate]);
        res.status(201).json({
