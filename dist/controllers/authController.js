...
export async function register(req, res) {
    try {
        ...
        await dbRun('INSERT INTO family_groups (id, name, created_by_user_id) VALUES (?, ?, ?)', [familyGroupId, `${fullName.trim()}'s Family`, userId]);
        await dbRun('INSERT INTO family_members (id, family_group_id, user_id, name, relationship, role, avatar_color) VALUES (?, ?, ?, ?, ?, ?, ?)', [familyMemberId, familyGroupId, userId, fullName.trim(), 'Primary Account Holder', 'OWNER', '#4f46e5']);
        // Insert default free subscription
        await dbRun('INSERT INTO subscriptions (id, user_id, plan_id, status) VALUES (?, ?, ?, ?)', [subscriptionId, userId, 'FREE', 'ACTIVE']);
        // Insert default notification preferences
        await dbRun('INSERT INTO notification_preferences (id, user_id) VALUES (?, ?)', [notifPrefId, userId]);
        // Record activity
...
export async function login(req, res) {
    try {
        ...
        const normalizedEmail = email.toLowerCase().trim();
        let user = await dbGet('SELECT id, email, password_hash, salt, is_admin FROM users WHERE LOWER(email) = ?', [normalizedEmail]);
...
export async function getMe(req, res) {
    try {
        ...
        const profile = await dbGet('SELECT full_name, avatar_url, phone, timezone, app_lock_enabled, app_lock_pin_hash, biometric_enabled FROM profiles WHERE user_id = ?', [userId]);
        const subscription = await dbGet('SELECT plan_id, status, current_period_end FROM subscriptions WHERE user_id = ?', [userId]);
        const familyMember = await dbGet('SELECT id, family_group_id, role FROM family_members WHERE user_id = ?', [userId]);
        const notifPref = await dbGet('SELECT in_app_enabled, browser_push_enabled, default_lead_days FROM notification_preferences WHERE user_id = ?', [userId]);
        // Count user documents
        const docCountRow = await dbGet('SELECT COUNT(*) as count FROM documents WHERE user_id = ?', [userId]);
...
export async function updateProfile(req, res) {
    try {
        ...
        await dbRun('UPDATE profiles SET full_name = ?, phone = ?, timezone = ?, avatar_url = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?', [fullName.trim(), phone || null, timezone || 'UTC', avatarUrl || null, userId]);
        // Also update family member name if owner
        await dbRun('UPDATE family_members SET name = ? WHERE user_id = ? AND role = "OWNER"', [fullName.trim(), userId]);
...
export async function setupAppLock(req, res) {
    try {
        const userId = req.user.id;
        ...
        if (enabled && pin) {
            ...
            await dbRun('UPDATE profiles SET app_lock_enabled = 1, app_lock_pin_hash = ?, biometric_enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?', [pinHash, biometricEnabled ? 1 : 0, userId]);
            res.json({ message: 'App Lock enabled successfully', appLockEnabled: true });
        }
        else if (!enabled) {
            await dbRun('UPDATE profiles SET app_lock_enabled = 0, app_lock_pin_hash = NULL, biometric_enabled = 0, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?', [userId]);
...
export async function verifyAppLock(req, res) {
    try {
        const userId = req.user.id;
        const { pin } = req.body;
        if (!pin) {
        ...
        const profile = await dbGet('SELECT app_lock_pin_hash FROM profiles WHERE user_id = ?', [userId]);
...
export async function updateNotificationPreferences(req, res) {
    try {
        ...
        await dbRun('UPDATE notification_preferences SET in_app_enabled = ?, browser_push_enabled = ?, default_lead_days = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?', [inAppEnabled ? 1 : 0, browserPushEnabled ? 1 : 0, leadDaysJson, userId]);
...
export async function deleteAccount(req, res) {
    try {
        ...
        const attachments = await dbAll(`SELECT a.file_path FROM document_attachments a
...
export async function forgotPassword(req, res) {
    try {
        ...
        await dbRun(`INSERT INTO password_resets (id, user_id, email, reset_code, token, used, expires_at)
       VALUES (?, ?, ?, ?, ?, 0, ?)`, [uuidv4(), user.id, normalizedEmail, resetCode, resetToken, expiresAt]);
        // Record activity
        await dbRun('INSERT INTO activity_history (id, user_id, action_type, description) VALUES (?, ?, ?, ?)', [uuidv4(), user.id, 'UPDATED', `Password reset code requested for ${normalizedEmail}`]);
...
export async function verifyResetCode(req, res) {
    try {
        ...
        const normalizedEmail = email.toLowerCase().trim();
        const resetRecord = await dbGet('SELECT id, token, expires_at, used FROM password_resets WHERE email = ? AND reset_code = ? AND used = 0 ORDER BY created_at DESC LIMIT 1', [normalizedEmail, resetCode.trim()]);
        if (!resetRecord) {
...
export async function resetPassword(req, res) {
    try {
        ...
        const normalizedEmail = email.toLowerCase().trim();
        const resetRecord = await dbGet('SELECT id, user_id, expires_at, used FROM password_resets WHERE email = ? AND reset_code = ? AND used = 0 ORDER BY created_at DESC LIMIT 1', [normalizedEmail, resetCode.trim()]);
        if (!resetRecord) {
        ...
        await dbRun('UPDATE users SET password_hash = ?, salt = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [hash, salt, resetRecord.user_id]);
        // Mark reset code as used
        await dbRun('UPDATE password_resets SET used = 1 WHERE id = ?', [resetRecord.id]);
        // Record activity
        await dbRun('INSERT INTO activity_history (id, user_id, action_type, description) VALUES (?, ?, ?, ?)', [uuidv4(), resetRecord.user_id, 'UPDATED', 'Account password was successfully reset']);
...
export async function submitContactSupport(req, res) {
    try {
