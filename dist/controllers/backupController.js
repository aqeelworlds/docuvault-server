...
export async function exportVaultData(req, res) {
    try {
        ...
        const renewalHistory = await dbAll(`SELECT rh.* FROM renewal_history rh
       ...
       WHERE d.user_id = ?`, [userId]);
        ...
        const familyMember = await dbGet('SELECT family_group_id FROM family_members WHERE user_id = ?', [userId]);
...
export async function syncVaultData(req, res) {
    try {
        const userId = req.user.id;
        const docRow = await dbGet('SELECT COUNT(*) as count FROM documents WHERE user_id = ? AND is_archived = 0', [userId]);
        const reminderRow = await dbGet('SELECT COUNT(*) as count FROM reminders WHERE user_id = ? AND is_active = 1', [userId]);
        const memberRow = await dbGet(`SELECT COUNT(*) as count FROM family_members 
       WHERE family_group_id = (SELECT family_group_id FROM family_members WHERE user_id = ?)`, [userId]);
...
export async function importVaultData(req, res) {
    try {
        const userId = req.user.id;
