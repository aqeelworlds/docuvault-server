 ...
 */
export async function checkUserIsPro(userId) {
...
export async function upgradeSubscription(req, res) {
    try {
        ...
        const existing = await dbGet('SELECT id FROM subscriptions WHERE user_id = ?', [userId]);
        if (existing) {
            await dbRun('UPDATE subscriptions SET plan_id = ?, status = "ACTIVE", current_period_end = ?, payment_provider = "DIRECT", updated_at = CURRENT_TIMESTAMP WHERE user_id = ?', [standardPlanId, periodEnd, userId]);
        }
        else {
            await dbRun('INSERT INTO subscriptions (id, user_id, plan_id, status, current_period_end, payment_provider) VALUES (?, ?, ?, "ACTIVE", ?, "DIRECT")', [uuidv4(), userId, standardPlanId, periodEnd]);
        }
        // Record activity
        await dbRun('INSERT INTO activity_history (id, user_id, action_type, description) VALUES (?, ?, ?, ?)', [uuidv4(), userId, 'UPDATED', `Upgraded to ${isLifetime ? 'Pro Lifetime' : isYearly ? 'Pro Yearly' : 'Pro Monthly'} plan`]);
        res.json({
 ...
 */
export async function verifyGooglePlayPurchase(req, res) {
    try {
        const userId = req.user.id;
        ...
        const existingTokenUser = await dbGet('SELECT user_id FROM subscriptions WHERE purchase_token = ? AND user_id != ?', [purchaseToken, userId]);
        ...
        const existingSub = await dbGet('SELECT id FROM subscriptions WHERE user_id = ?', [userId]);
        if (existingSub) {
            await dbRun(`UPDATE subscriptions SET
          plan_id = ?, status = "ACTIVE", current_period_end = ?,
          payment_provider = "GOOGLE_PLAY", order_id = ?, purchase_token = ?,
          updated_at = CURRENT_TIMESTAMP
         WHERE user_id = ?`, [standardPlanId, periodEnd, orderId || 'GPA.' + Date.now(), purchaseToken, userId]);
        }
        else {
            await dbRun(`INSERT INTO subscriptions (
          id, user_id, plan_id, status, current_period_end,
          payment_provider, order_id, purchase_token
        ) VALUES (?, ?, ?, "ACTIVE", ?, "GOOGLE_PLAY", ?, ?)`, [uuidv4(), userId, standardPlanId, periodEnd, orderId || 'GPA.' + Date.now(), purchaseToken]);
        }
        // Record activity
        await dbRun('INSERT INTO activity_history (id, user_id, action_type, description) VALUES (?, ?, ?, ?)', [uuidv4(), userId, 'UPDATED', `Verified Google Play purchase for ${standardPlanId}`]);
        res.json({
            verified: true,
