...
export async function getDocuments(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const userMember = await dbGet<{ id: string }>('SELECT id FROM family_members WHERE user_id = ?', [userId]);
    ...
    const rawDocs = await dbAll<any>(sql, [userId, userMemberId, userId, userMemberId]);
    ...
    filtered.sort((a, b) => {
      if (sortBy === 'expiry_asc') {
        if (a.has_no_expiry && !b.has_no_expiry) return 1;
        if (!a.has_no_expiry && b.has_no_expiry) return -1;
...
export async function getDocumentById(req: AuthorizedDocumentRequest, res: Response): Promise<void> {
  try {
    ...
    const attachments = await dbAll(
      'SELECT id, file_name, file_size, mime_type, is_primary, created_at FROM document_attachments WHERE document_id = ? ORDER BY is_primary DESC, created_at ASC',
      [documentId]
    );

    // Get active reminders
    const reminders = await dbAll(
      'SELECT id, lead_days, reminder_date, is_active, is_triggered FROM reminders WHERE document_id = ? ORDER BY lead_days DESC',
      [documentId]
    );

    // Get renewal history
    const renewalHistory = await dbAll(
      `SELECT rh.*, u.email as renewed_by_email, p.full_name as renewed_by_name
    ...
    const activity = await dbAll(
      `SELECT ah.*, p.full_name as user_name
    ...
    let permissions: any[] = [];
    if (access.isOwner) {
      permissions = await dbAll(
        `SELECT dp.*, fm.name as member_name, fm.relationship, fm.avatar_color
...
export async function createDocument(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    if (!isPro) {
      const docCountRow = await dbGet<{ count: number }>('SELECT COUNT(*) as count FROM documents WHERE user_id = ? AND is_archived = 0', [userId]);
    ...
    const userMember = await dbGet<{ id: string; family_group_id: string }>(
    ...
    const existingType = await dbGet<{ id: string }>(
      'SELECT id FROM document_types WHERE id = ? OR slug = ?',
    ...
    if (!isNoExpiry && expiryDate) {
      let leadDaysList: number[] = [90, 60, 30, 14, 7, 1];
      if (customLeadDays) {
        try {
      ...
      for (const rem of remindersToCreate) {
        await dbRun(
          ...
          [uuidv4(), documentId, userId, rem.leadDays, rem.reminderDate]
        );
      }
    }
    ...
    await dbRun(
      ...
      [uuidv4(), documentId, userId, 'CREATED', `Added document "${name.trim()}"`]
    );
...
export async function updateDocument(req: AuthorizedDocumentRequest, res: Response): Promise<void> {
  try {
    const documentId = req.params.id;
    const userId = req.user!.id;
    const {
    ...
    if (expiryChanged) {
      if (isNoExpiry) {
        ...
        const existingReminders = await dbAll<{ id: string; lead_days: number }>(
          'SELECT id, lead_days FROM reminders WHERE document_id = ?',
          [documentId]
        );
        ...
        if (existingReminders.length > 0) {
          ...
          for (const rem of newDates) {
            await dbRun(
              'UPDATE reminders SET reminder_date = ?, is_active = 1, is_triggered = 0 WHERE document_id = ? AND lead_days = ?',
              [rem.reminderDate, documentId, rem.leadDays]
            );
          }
        } else {
    ...
    await dbRun(
      'INSERT INTO activity_history (id, document_id, user_id, action_type, description) VALUES (?, ?, ?, ?, ?)',
      [uuidv4(), documentId, userId, 'UPDATED', `Updated details for "${name.trim()}"`]
    );
...
export async function archiveDocument(req: AuthorizedDocumentRequest, res: Response): Promise<void> {
  try {
    const documentId = req.params.id;
    const userId = req.user!.id;
    ...
    await dbRun(
      'UPDATE documents SET is_archived = 1, archived_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [documentId]
    );
    ...
    await dbRun(
      'INSERT INTO activity_history (id, document_id, user_id, action_type, description) VALUES (?, ?, ?, ?, ?)',
      [uuidv4(), documentId, userId, 'UPDATED', `Archived document "${docName}"`]
    );
...
export async function unarchiveDocument(req: AuthorizedDocumentRequest, res: Response): Promise<void> {
  try {
    const documentId = req.params.id;
    const userId = req.user!.id;
    ...
    await dbRun(
      'UPDATE documents SET is_archived = 0, archived_at = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [documentId]
    );
    ...
    await dbRun(
      'INSERT INTO activity_history (id, document_id, user_id, action_type, description) VALUES (?, ?, ?, ?, ?)',
      [uuidv4(), documentId, userId, 'UPDATED', `Unarchived document "${doc.name}"`]
    );
...
export async function renewDocument(req: AuthorizedDocumentRequest, res: Response): Promise<void> {
  try {
    const documentId = req.params.id;
    const userId = req.user!.id;
    const {
    ...
    await dbRun(
      'INSERT INTO activity_history (id, document_id, user_id, action_type, description) VALUES (?, ?, ?, ?, ?)',
      [uuidv4(), documentId, userId, 'RENEWED', `Renewed document from ${previousDoc.expiry_date || 'N/A'} to ${newExpiryDate}`]
    );
...
export async function deleteDocument(req: AuthorizedDocumentRequest, res: Response): Promise<void> {
  try {
    const documentId = req.params.id;
    const userId = req.user!.id;
    const docName = req.docAccess?.document?.name || 'Document';
    ...
    const attachments = await dbAll<{ file_path: string }>(
      'SELECT file_path FROM document_attachments WHERE document_id = ?',
      [documentId]
    );
    ...
    await dbRun(
      ...
      [uuidv4(), userId, 'UPDATED', `Deleted document "${docName}"`]
    );
