...
export async function getFamilyGroup(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    const requestedGroupId = req.query.familyGroupId as string;

    // Find family member entry for current user (prefer joined family if non-owner, or requested group)
    let member;
    if (requestedGroupId) {
      member = await dbGet<{ family_group_id: string; role: string; id: string }>(
        'SELECT id, family_group_id, role FROM family_members WHERE user_id = ? AND family_group_id = ?',
        [userId, requestedGroupId]
      );
    } else {
      member = await dbGet<{ family_group_id: string; role: string; id: string }>(
        `SELECT id, family_group_id, role FROM family_members
         WHERE user_id = ?
         ORDER BY (CASE WHEN role != 'OWNER' THEN 0 ELSE 1 END), created_at DESC`,
        [userId]
      );
    }
    ...
    const familyGroup = await dbGet<{ id: string; name: string; created_by_user_id: string; created_at: string }>(
    ...
    const familyDocs = await dbAll<any>(
      `SELECT DISTINCT
    ...
    const enrichedFamilyDocs = familyDocs.map(doc => {
      ...
      if (metrics.status === 'EXPIRING_SOON') expiringSoonCount++;
      if (metrics.status === 'EXPIRED') expiredCount++;
      if (metrics.status === 'ACTIVE' || metrics.status === 'LIFETIME') activeCount++;
    ...
    const sharedPerms = await dbAll<any>(
      `SELECT 
    ...
    const sentInvitations = await dbAll<any>(
      `SELECT fi.*, p.full_name as inviter_name
       FROM family_invitations fi
       LEFT JOIN profiles p ON fi.invited_by_user_id = p.user_id
       WHERE fi.family_group_id = ? AND fi.status = 'PENDING' AND fi.expires_at > CURRENT_TIMESTAMP`,
      [member.family_group_id]
    );
...
export async function updateFamilyGroupName(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    const { name } = req.body;
    ...
    const member = await dbGet<{ family_group_id: string; role: string }>(
      'SELECT family_group_id, role FROM family_members WHERE user_id = ?',
      [userId]
    );
...
export async function addFamilyMember(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const selfMember = await dbGet<{ family_group_id: string; role: string }>(
      'SELECT family_group_id, role FROM family_members WHERE user_id = ?',
      [userId]
    );
    ...
    const countRow = await dbGet<{ count: number }>(
      'SELECT COUNT(*) as count FROM family_members WHERE family_group_id = ?',
    ...
    let invitationId: string = uuidv4();
    ...
    if (email && typeof email === 'string' && email.includes('@')) {
      ...
      const existingInFamily = await dbGet(
        'SELECT id FROM family_members WHERE family_group_id = ? AND (email = ? OR user_id IN (SELECT id FROM users WHERE email = ?))',
        [selfMember.family_group_id, normalizedEmail, normalizedEmail]
      );
      if (existingInFamily) {
      ...
      const existingUser = await dbGet<{ id: string }>('SELECT id FROM users WHERE email = ?', [normalizedEmail]);
    ...
    await dbRun(
      `INSERT INTO family_members (id, family_group_id, user_id, name, email, relationship, role, avatar_color, status, invitation_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        memberId,
        selfMember.family_group_id,
    ...
    await dbRun(
      ...
      [uuidv4(), userId, 'SHARED', `Added family member ${name.trim()} (${relationship.trim()})`]
    );
...
export async function joinFamilyByCode(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const invitation = await dbGet<any>(
      `SELECT fi.*, fg.name as family_name 
       FROM family_invitations fi 
       JOIN family_groups fg ON fi.family_group_id = fg.id 
       WHERE (UPPER(fi.invite_code) = ? OR UPPER(fi.id) = ?) AND fi.status = 'PENDING'`,
      [cleanCode, cleanCode]
    );
    ...
    const existingMember = await dbGet<{ id: string }>(
      'SELECT id FROM family_members WHERE invitation_id = ? OR (family_group_id = ? AND (LOWER(email) = ? OR user_id = ?))',
      [invitation.id, invitation.family_group_id, userEmail.toLowerCase(), userId]
    );

    if (existingMember) {
      await dbRun(
        'UPDATE family_members SET user_id = ?, name = ?, email = ?, status = "ACTIVE" WHERE id = ?',
        [userId, userName || 'Family Member', userEmail.toLowerCase(), existingMember.id]
      );
    } else {
      const memberId = uuidv4();
      await dbRun(
    ...
    await dbRun(
      'UPDATE family_invitations SET status = "ACCEPTED", invitee_user_id = ?, invitee_email = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [userId, userEmail.toLowerCase(), invitation.id]
    );

    await dbRun(
      'INSERT INTO activity_history (id, user_id, action_type, description) VALUES (?, ?, ?, ?)',
      [uuidv4(), userId, 'UPDATED', `Joined ${invitation.family_name} via invite code`]
    );
...
export async function updateFamilyMember(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const callerMember = await dbGet<{ id: string; role: string; family_group_id: string }>(
      'SELECT id, role, family_group_id FROM family_members WHERE user_id = ?',
      [userId]
    );
    ...
    const targetMember = await dbGet<{ id: string; family_group_id: string }>(
      'SELECT id, family_group_id FROM family_members WHERE id = ?',
      [memberId]
    );
    ...
    await dbRun(
      'UPDATE family_members SET name = ?, relationship = ?, avatar_color = COALESCE(?, avatar_color) WHERE id = ?',
      [name.trim(), relationship ? relationship.trim() : 'Family Member', avatarColor || null, memberId]
    );
...
export async function deleteFamilyMember(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const selfMember = await dbGet<{ id: string; role: string; family_group_id: string }>(
      'SELECT id, role, family_group_id FROM family_members WHERE user_id = ?',
      [userId]
    );
    ...
    const memberToDelete = await dbGet<{ id: string; invitation_id: string }>('SELECT id, invitation_id FROM family_members WHERE id = ?', [memberId]);
...
export async function inviteFamilyMember(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    ...
    const existingUser = await dbGet<{ id: string; full_name: string }>(
      'SELECT u.id, p.full_name FROM users u LEFT JOIN profiles p ON u.id = p.user_id WHERE u.email = ?',
      [normalizedEmail]
    );

    if (existingUser) {
      const alreadyMember = await dbGet(
        'SELECT id FROM family_members WHERE family_group_id = ? AND (user_id = ? OR email = ?)',
        [selfMember.family_group_id, existingUser.id, normalizedEmail]
      );
      if (alreadyMember) {
...
export async function getPendingInvitations(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const invitations = await dbAll<any>(
      `SELECT fi.*, fg.name as family_name, p.full_name as inviter_name, u.email as inviter_email
       FROM family_invitations fi
       JOIN family_groups fg ON fi.family_group_id = fg.id
       JOIN users u ON fi.invited_by_user_id = u.id
       LEFT JOIN profiles p ON u.id = p.user_id
       WHERE fi.invitee_email = ? AND fi.status = 'PENDING' AND fi.expires_at > CURRENT_TIMESTAMP`,
      [userEmail.toLowerCase()]
    );
...
export async function acceptInvitation(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const invitation = await dbGet<any>(
      'SELECT * FROM family_invitations WHERE id = ? AND invitee_email = ? AND status = "PENDING"',
      [invitationId, userEmail.toLowerCase()]
    );
    ...
    // Check if member row already existed for this invitation
    const existingMember = await dbGet<{ id: string }>(
      'SELECT id FROM family_members WHERE invitation_id = ? OR (family_group_id = ? AND email = ?)',
      [invitationId, invitation.family_group_id, userEmail.toLowerCase()]
    );

    if (existingMember) {
      await dbRun(
        'UPDATE family_members SET user_id = ?, name = ?, status = "ACTIVE" WHERE id = ?',
        [userId, userName || 'Family Member', existingMember.id]
      );
    } else {
      const memberId = uuidv4();
      await dbRun(
    ...
    // Update invitation status
    await dbRun(
      'UPDATE family_invitations SET status = "ACCEPTED", invitee_user_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [userId, invitationId]
    );

    await dbRun(
      'INSERT INTO activity_history (id, user_id, action_type, description) VALUES (?, ?, ?, ?)',
      [uuidv4(), userId, 'UPDATED', `Accepted family invitation from ${invitation.invited_by_user_id}`]
    );
...
export async function rejectInvitation(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    await dbRun(
      'UPDATE family_invitations SET status = "REJECTED", updated_at = CURRENT_TIMESTAMP WHERE id = ? AND invitee_email = ?',
      [invitationId, userEmail.toLowerCase()]
    );

    await dbRun(
      'UPDATE family_members SET status = "REJECTED" WHERE invitation_id = ?',
      [invitationId]
    );
...
export async function cancelInvitation(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    await dbRun(
      'UPDATE family_invitations SET status = "CANCELLED", updated_at = CURRENT_TIMESTAMP WHERE id = ? AND family_group_id = ?',
      [invitationId, selfMember.family_group_id]
    );

    await dbRun(
      'DELETE FROM family_members WHERE invitation_id = ? AND status = "PENDING"',
      [invitationId]
    );
...
export async function leaveFamily(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    ...
    let member;
    if (familyGroupId) {
      member = await dbGet<{ id: string; role: string; family_group_id: string }>(
        'SELECT id, role, family_group_id FROM family_members WHERE user_id = ? AND family_group_id = ?',
        [userId, familyGroupId]
      );
    } else {
      member = await dbGet<{ id: string; role: string; family_group_id: string }>(
        'SELECT id, role, family_group_id FROM family_members WHERE user_id = ? AND role != "OWNER"',
        [userId]
      );
    }
...
export async function deleteFamilyGroup(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    ...
    await dbRun(
      `DELETE FROM document_permissions WHERE document_id IN (SELECT id FROM documents WHERE user_id = ?)`,
      [userId]
    );
...
export async function shareDocumentWithMember(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    ...
    const doc = await dbGet<{ id: string; user_id: string; name: string }>(
      'SELECT id, user_id, name FROM documents WHERE id = ?',
      [documentId]
    );
    ...
    const existingPerm = await dbGet<{ id: string }>(
      'SELECT id FROM document_permissions WHERE document_id = ? AND shared_with_member_id = ?',
      [documentId, memberId]
    );

    if (existingPerm) {
      await dbRun(
        'UPDATE document_permissions SET permission_level = ?, granted_by_user_id = ? WHERE id = ?',
        [level, userId, existingPerm.id]
      );
    } else {
      await dbRun(
        'INSERT INTO document_permissions (id, document_id, shared_with_member_id, permission_level, granted_by_user_id) VALUES (?, ?, ?, ?, ?)',
        [uuidv4(), documentId, memberId, level, userId]
      );
    }

    // Log activity
    await dbRun(
      'INSERT INTO activity_history (id, document_id, user_id, action_type, description) VALUES (?, ?, ?, ?, ?)',
      [uuidv4(), documentId, userId, 'SHARED', `Shared document "${doc.name}" (${level} access)`]
    );
...
export async function unshareDocumentFromMember(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    const documentId = req.body.documentId || req.body.document_id;
    const memberId = req.body.memberId || req.body.member_id;
    ...
    const doc = await dbGet<{ id: string; user_id: string }>(
    ...
    await dbRun(
      'DELETE FROM document_permissions WHERE document_id = ? AND shared_with_member_id = ?',
      [documentId, memberId]
    );
