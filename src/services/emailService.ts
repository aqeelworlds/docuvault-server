 ...
 */
export async function sendPasswordResetOtp(toEmail: string, otpCode: string): Promise<boolean> {
  const config = getEmailConfig();
 ...
 */
export async function sendSupportInquiry(name: string, userEmail: string, userMessage: string): Promise<boolean> {
  const config = getEmailConfig();
  const transporter = createTransporter();
