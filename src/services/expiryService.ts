export type ExpiryStatus = 'ACTIVE' | 'EXPIRING_SOON' | 'EXPIRED' | 'LIFETIME';

export interface ExpiryCalculationResult {
  status: ExpiryStatus;
  daysRemaining: number | null;
  urgencyLevel: 'safe' | 'warning' | 'urgent' | 'expired' | 'neutral';
 ...
 */
export function normalizeToCalendarDate(dateInput: string | Date): Date {
 ...
 */
export function differenceInCalendarDays(targetDate: string | Date, baseDate: string | Date = new Date()): number {
 ...
 */
export function calculateExpiryMetrics(
  ...
  if (daysRemaining <= warningThresholdDays) {
    let text = '';
    let urgency: 'warning' | 'urgent' = 'warning';
...
export function generateReminderDates(
  ...
  leadDaysList: number[] = [90, 60, 30, 14, 7, 1]
): Array<{ leadDays: number; reminderDate: string }> {
