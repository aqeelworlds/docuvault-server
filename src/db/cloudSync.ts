...
import { dbAll, dbRun, dbGet, initDatabase } from './database.js';
...
function httpsRequest(options: https.RequestOptions, postData?: string): Promise<{ statusCode: number; data: string }> {
  return new Promise((resolve, reject) => {
    const headers: Record<string, any> = { ...(options.headers as Record<string, any> || {}) };
 ...
 */
export async function pullCloudDatabase(): Promise<boolean> {
  if (isSyncing) return false;
  isSyncing = true;

  try {
    // 1. Try pulling raw JSON directly from GitHub Raw
    const rawOptions: https.RequestOptions = {
      hostname: 'raw.githubusercontent.com',
      path: `/${GITHUB_REPO}/main/${DATA_FILE_PATH}`,
      method: 'GET',
      headers: {
    ...
    const rawRes = await httpsRequest(rawOptions);
    if (rawRes.statusCode === 200 && rawRes.data) {
      try {
        const dbSnapshot = JSON.parse(rawRes.data);
        if (dbSnapshot && dbSnapshot.tables) {
          await restoreSnapshot(dbSnapshot);
          lastPullTimestamp = Date.now();
          console.log('[CloudSync] Successfully synced database from GitHub Cloud (Raw).');
          return true;
        }
      } catch (parseErr) {
        console.warn('[CloudSync] JSON parse error from raw:', parseErr);
      }
    }
    ...
    const options: https.RequestOptions = {
      ...
      path: `/repos/${GITHUB_REPO}/contents/${DATA_FILE_PATH}?ref=main`,
      method: 'GET',
      headers: {
        'User-Agent': 'DocuVault-Backend',
        'Authorization': `token ${GITHUB_TOKEN}`,
    ...
    if (res.statusCode === 200) {
      ...
      if (json.content) {
        ...
        await restoreSnapshot(dbSnapshot);
        lastPullTimestamp = Date.now();
        console.log('[CloudSync] Successfully synced database from GitHub Cloud (API).');
        return true;
      }
 ...
 */
export async function pushCloudDatabase(): Promise<boolean> {
  try {
...
export async function syncToCloudNow(): Promise<boolean> {
  if (pushTimeout) {
    clearTimeout(pushTimeout);
    pushTimeout = null;
  }
  return await pushCloudDatabase();
}

export function queueCloudSync(): void {
  // Trigger immediate push for serverless safety
...
async function exportSnapshot(): Promise<any> {
  ...
  const documentPermissions = await dbAll('SELECT * FROM document_permissions');
  ...
  try {
    ...
    if (Array.isArray(t.users) && t.users.length > 0) {
      ...
      for (const u of t.users) {
        await dbRun(
          ...
          [u.id, u.email, u.password_hash, u.salt, u.is_admin || 0, u.last_login_at || null, u.created_at, u.updated_at]
        );
      }
    }
    ...
    if (Array.isArray(t.profiles)) {
      for (const p of t.profiles) {
        await dbRun(
          'INSERT OR REPLACE INTO profiles (id, user_id, full_name, avatar_url, phone, timezone, app_lock_enabled, app_lock_pin_hash, biometric_enabled, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [p.id, p.user_id, p.full_name, p.avatar_url, p.phone, p.timezone || 'UTC', p.app_lock_enabled || 0, p.app_lock_pin_hash || null, p.biometric_enabled || 0, p.created_at, p.updated_at]
        );
      }
    }
    ...
    if (Array.isArray(t.subscriptions)) {
      for (const s of t.subscriptions) {
        await dbRun(
          'INSERT OR REPLACE INTO subscriptions (id, user_id, plan_id, status, payment_provider, current_period_start, current_period_end, cancel_at_period_end, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [s.id, s.user_id, s.plan_id || 'FREE', s.status || 'ACTIVE', s.payment_provider || 'DIRECT', s.current_period_start, s.current_period_end, s.cancel_at_period_end || 0, s.created_at, s.updated_at]
        );
      }
    }
    ...
    if (Array.isArray(t.family_groups || t.familyGroups)) {
      for (const fg of (t.family_groups || t.familyGroups)) {
        await dbRun(
          'INSERT OR REPLACE INTO family_groups (id, name, created_by_user_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?)',
          [fg.id, fg.name, fg.created_by_user_id, fg.created_at, fg.updated_at]
        );
      }
    }
    ...
    if (Array.isArray(t.family_members || t.familyMembers)) {
      for (const fm of (t.family_members || t.familyMembers)) {
        await dbRun(
          'INSERT OR REPLACE INTO family_members (id, family_group_id, user_id, name, relationship, role, avatar_color, email, status, invitation_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [fm.id, fm.family_group_id, fm.user_id, fm.name, fm.relationship, fm.role, fm.avatar_color, fm.email, fm.status || 'ACTIVE', fm.invitation_id, fm.created_at]
        );
      }
    }
    ...
    if (Array.isArray(t.documents)) {
      for (const d of t.documents) {
        await dbRun(
          'INSERT OR REPLACE INTO documents (id, user_id, family_group_id, owner_member_id, document_type_id, name, document_number, issue_date, expiry_date, has_no_expiry, issuing_authority, notes, is_archived, archived_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [d.id, d.user_id, d.family_group_id, d.owner_member_id, d.document_type_id, d.name, d.document_number, d.issue_date, d.expiry_date, d.has_no_expiry || 0, d.issuing_authority, d.notes, d.is_archived || 0, d.archived_at, d.created_at, d.updated_at]
        );
      }
    }
    ...
    if (Array.isArray(t.attachments)) {
      for (const a of t.attachments) {
        await dbRun(
          'INSERT OR REPLACE INTO document_attachments (id, document_id, file_name, file_path, file_size, mime_type, is_primary, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          [a.id, a.document_id, a.file_name, a.file_path, a.file_size, a.mime_type, a.is_primary || 0, a.created_at]
        );
      }
    }
    ...
    if (Array.isArray(t.reminders)) {
      for (const r of t.reminders) {
        await dbRun(
          'INSERT OR REPLACE INTO reminders (id, document_id, reminder_date, lead_days, is_sent, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
          [r.id, r.document_id, r.reminder_date, r.lead_days, r.is_sent || 0, r.is_active || 1, r.created_at]
        );
      }
    }

    if (Array.isArray(t.document_permissions || t.documentPermissions)) {
      for (const dp of (t.document_permissions || t.documentPermissions)) {
        await dbRun(
          'INSERT OR REPLACE INTO document_permissions (id, document_id, shared_with_member_id, permission_level, granted_by_user_id, created_at) VALUES (?, ?, ?, ?, ?, ?)',
          [dp.id, dp.document_id, dp.shared_with_member_id, dp.permission_level || 'VIEW', dp.granted_by_user_id, dp.created_at]
        );
      }
    }
    ...
    if (Array.isArray(t.activityHistory || t.activity_history)) {
      for (const ah of (t.activityHistory || t.activity_history)) {
        await dbRun(
          'INSERT OR REPLACE INTO activity_history (id, user_id, action_type, description, created_at) VALUES (?, ?, ?, ?, ?)',
          [ah.id, ah.user_id, ah.action_type, ah.description, ah.created_at]
        );
      }
    }
