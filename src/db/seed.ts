...
export async function seedDemoData() {
  ...
  const existingUser = await dbGet<{ id: string }>('SELECT id FROM users WHERE email = ?', [demoEmail]);
  if (existingUser) {
  ...
  await dbRun(
    ...
    [userId, demoEmail, hash, salt]
  );

  // Insert Profile with App Lock PIN '1234' configured
  await dbRun(
    `INSERT INTO profiles (id, user_id, full_name, timezone, app_lock_enabled, app_lock_pin_hash, biometric_enabled)
     VALUES (?, ?, ?, 'America/New_York', 1, ?, 1)`,
    [profileId, userId, 'Alexander Wright', pinHash]
  );

  // Insert Pro Subscription
  await dbRun(
    `INSERT INTO subscriptions (id, user_id, plan_id, status, current_period_end)
     VALUES (?, ?, 'PRO_YEARLY', 'ACTIVE', '2027-08-23T00:00:00Z')`,
    [uuidv4(), userId]
  );

  // Insert Notification Preferences
  await dbRun(
    'INSERT INTO notification_preferences (id, user_id, default_lead_days) VALUES (?, ?, ?)',
    [uuidv4(), userId, '[90, 60, 30, 14, 7, 1]']
  );

  // Insert Family Group
  await dbRun(
    'INSERT INTO family_groups (id, name, created_by_user_id) VALUES (?, ?, ?)',
    [familyGroupId, 'Wright Family Vault', userId]
  );

  // Insert Family Members
  await dbRun(
    'INSERT INTO family_members (id, family_group_id, user_id, name, relationship, role, avatar_color) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [ownerMemberId, familyGroupId, userId, 'Alexander Wright', 'Self (Primary)', 'OWNER', '#4f46e5']
  );

  await dbRun(
    'INSERT INTO family_members (id, family_group_id, name, relationship, role, avatar_color) VALUES (?, ?, ?, ?, ?, ?)',
    [spouseMemberId, familyGroupId, 'Elena Wright', 'Spouse', 'ADMIN', '#ec4899']
  );

  await dbRun(
    'INSERT INTO family_members (id, family_group_id, name, relationship, role, avatar_color) VALUES (?, ?, ?, ?, ?, ?)',
    [childMemberId, familyGroupId, 'Leo Wright', 'Child', 'MEMBER', '#3b82f6']
  );

  await dbRun(
    'INSERT INTO family_members (id, family_group_id, name, relationship, role, avatar_color) VALUES (?, ?, ?, ?, ?, ?)',
    [parentMemberId, familyGroupId, 'Margaret Wright', 'Mother', 'MEMBER', '#10b981']
  );
  ...
  for (const item of seedDocs) {
    ...
    if (item.ownerId === ownerMemberId && item.name.includes('Passport')) {
      await dbRun(
        'INSERT INTO document_permissions (id, document_id, shared_with_member_id, permission_level, granted_by_user_id) VALUES (?, ?, ?, "VIEW", ?)',
        [uuidv4(), docId, spouseMemberId, userId]
      );
    }
    ...
    if (!item.hasNoExpiry && item.expiry) {
      ...
      for (const rem of reminders) {
        await dbRun(
          ...
          [uuidv4(), docId, userId, rem.leadDays, rem.reminderDate]
        );
      }
    }
  }
