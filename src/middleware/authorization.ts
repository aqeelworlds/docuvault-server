...
export interface AuthorizedDocumentRequest extends AuthenticatedRequest {
 ...
 */
export async function getDocumentAccess(userId: string, documentId: string): Promise<DocumentAccessContext | null> {
  const doc = await dbGet<any>(
    `SELECT d.*, dt.name as category_name, dt.slug as category_slug, dt.icon as category_icon, dt.color as category_color,
  ...
  const userObj = await dbGet<{ id: string; email: string }>('SELECT id, email FROM users WHERE id = ?', [userId]);
  const userEmail = userObj?.email ? userObj.email.toLowerCase().trim() : '';

  // Auto-link any unlinked family_members with this email
  if (userEmail) {
    await dbRun(
      'UPDATE family_members SET user_id = ? WHERE (user_id IS NULL OR user_id = "") AND LOWER(email) = ?',
      [userId, userEmail]
    );
  }
  ...
  if (doc.family_group_id) {
    const ownerMember = await dbGet<{ id: string; role: string }>(
      'SELECT id, role FROM family_members WHERE family_group_id = ? AND (user_id = ? OR (email IS NOT NULL AND LOWER(email) = ?)) AND role = "OWNER"',
      [doc.family_group_id, userId, userEmail]
    );
    if (ownerMember) {
      return {
        documentId,
  ...
  const perm = await dbGet<{ permission_level: string }>(
    `SELECT dp.permission_level
     ...
     ORDER BY CASE WHEN dp.permission_level = 'EDIT' THEN 1 ELSE 2 END ASC`,
    [documentId, userId, userEmail]
  );
  ...
  if (perm && (perm.permission_level === 'VIEW' || perm.permission_level === 'EDIT')) {
    return {
      documentId,
      isOwner: false,
      permissionLevel: perm.permission_level as 'VIEW' | 'EDIT',
      document: doc
    };
  }
  ...
  if (doc.owner_member_id) {
    const isOwnerMember = await dbGet<{ id: string }>(
      'SELECT id FROM family_members WHERE id = ? AND (user_id = ? OR (email IS NOT NULL AND LOWER(email) = ?))',
      [doc.owner_member_id, userId, userEmail]
    );
    if (isOwnerMember) {
      return {
        documentId,
 ...
 */
export async function requireDocumentView(req: AuthorizedDocumentRequest, res: Response, next: NextFunction): Promise<void> {
 ...
 */
export async function requireDocumentEdit(req: AuthorizedDocumentRequest, res: Response, next: NextFunction): Promise<void> {
  const documentId = String(req.params.id || req.params.documentId || '');
  const userId = req.user?.id;
 ...
 */
export async function requireDocumentOwner(req: AuthorizedDocumentRequest, res: Response, next: NextFunction): Promise<void> {
  const documentId = String(req.params.id || req.params.documentId || '');
  const userId = req.user?.id;
